...
// Verify branch manager by email for a given outlet
router.post('/verify-branch-manager', async (req, res) => {
  try {
    const { outletId, email } = req.body || {}
    if (!outletId || !email) return res.status(400).json({ error: 'outletId and email are required' })

    const outlet = await prisma.outlet.findUnique({ where: { id: outletId }, include: { region: true } })
    if (!outlet) return res.status(404).json({ error: 'Outlet not found' })
    if (!outlet.region) return res.status(404).json({ error: 'Region not found for outlet' })

    const configured = (outlet.region.managerEmail || '').trim().toLowerCase()
    const provided = String(email).trim().toLowerCase()
    if (!configured) return res.status(403).json({ error: 'No manager email configured for this branch/region' })

    if (configured === provided) {
      return res.json({ success: true })
    }
    return res.status(401).json({ error: 'Manager email does not match' })
  } catch (error) {
    console.error('verify-branch-manager error:', error)
